...
export default function SingerPortfolio() {
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    // Template params with dynamic 'to_email' (to the email entered by the user)
    const templateParams = {
      from_name: contactForm.name, // Sender's name
      from_email: contactForm.email, // Sender's email (could be yours or from the form)
      message: contactForm.message, // Message content
      to_email: contactForm.email, // To the entered email (recipient)
    };

    try {
      // This email will only go to the email entered in the 'contactForm.email'
      const response = await emailjs.send(
        "service_mfk0fir", // Your service ID (this stays the same)
        "template_smao2bq", // Your template ID (this stays the same)
        templateParams, // Dynamic email parameters
        "_AH5b_SlDTSu0DHfO" // Your user ID (this stays the same)
      );
